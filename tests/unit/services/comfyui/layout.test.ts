...
import { extractGraphEdges, extractGraphNodes, layoutWorkflow, readNodeDisplayName } from '@/services/comfyui/layout';
...
describe('comfyui layout', () => {
  it('reads node display names with fallback', () => {
    expect(readNodeDisplayName({ class_type: 'CLIPTextEncode', inputs: {}, _meta: { title: ' My Prompt ' } }, '1')).toBe('My Prompt');
    expect(readNodeDisplayName({ class_type: 'CheckpointLoader', inputs: {} }, '2')).toBe('CheckpointLoader');
    expect(readNodeDisplayName({ class_type: '', inputs: {} }, '3')).toBe('3');
  });
