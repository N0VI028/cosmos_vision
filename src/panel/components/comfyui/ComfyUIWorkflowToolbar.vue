<template>
  <div class="flex flex-col gap-(--cv-space-sm)">
    <div class="grid grid-cols-[repeat(auto-fit,minmax(10rem,1fr))] gap-(--cv-space-lg)">
      <Button label="导入 API JSON" icon="fa-solid fa-download" @click="emit('import')" />
      <Button
...
const statusClass = computed(() => {
  if (props.statusTone === 'error') return 'text-(--cvp-red-500,var(--cv-on-surface))';
  if (props.statusTone === 'warn') return 'text-(--cvp-orange-500,var(--cv-on-surface))';
